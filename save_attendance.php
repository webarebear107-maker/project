...
if (isset($_POST['qr_code'])) {
    ...
    if (mysqli_num_rows($check) > 0) {
        ...
        if (mysqli_num_rows($duplicate) == 0) {
            mysqli_query(
                $conn,
                "INSERT INTO time_in (lrn, fullname, level, date_logged, time_logged)
                 VALUES ('$lrn', '$fullname', '$level', '$today', '$time')"
            );

            header("Location: time_in.php?success=recorded");
            exit();
        } else {
            header("Location: time_in.php?error=duplicate");
            exit();
        }
    } else {
        header("Location: time_in.php?error=invalid");
        exit();
    }
}
?>
